...
This script computes the top-N matches for a given partner
 ...
 usage : ./knn.py --input /opt/output-per-partner/1458655100/20585/cnn_features.txt.gz
"""
...
def main():
    ...
    parser.add_argument('--nblocks', dest='n_blocks', type=int, required=False)
    parser.add_argument('--type', dest='feature_type', type=str, required=True)
    ...
    partnerid = int(re.findall('[0-9]+', args.input_file)[-1])

    # extract timestamp
    timestamp = int(re.findall('[0-9]+', args.input_file)[-2])
    ...
    if args.feature_type == 'cnn':
        shrink_factors = [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 0]
    else:
        shrink_factors = [2, 4, 8, 16, 32, 64,
                          128, 256, 512, 1024, 2048, 4096, 0]
    if args.feature_type == 'cnn':
        shrink_factors = [2, 8, 32, 64, 128, 1024, 0]
    else:
        shrink_factors = [2, 8, 32, 64, 128, 1024, 0]
    # if args.feature_type == 'cnn':
    #    shrink_factors = [128,1024,0]
    # else:
    #    shrink_factors = [128,1024,0]

    if not args.n_blocks:
        n_blocks = 1
    else:
    ...
    for shrink in shrink_factors:
        # main run
        run(files, partnerid, n_blocks, timestamp, shrink, args.feature_type)
...
def run(files, partnerid, nx, timestamp, shrink, feature_type):
    ...
    for b in range(n_batches):
        ...
        for c in range(32):
            ...
            process = multiprocessing.Process(target=compute_matches, args=[
                                              ii, ij, files[ii], files[ij], topn, shrink, feature_type, return_dict])
            process.start()
            jobs.append(process)
        ...
        for (key, val) in return_dict.items():
            matches[key] = val[0].copy()
            if feature_length == 0:
                feature_length = val[1]
            else:
                assert (feature_length == val[1])

    topn_matrix = build_topn(matches, files, nx, nf, topn,
                             sampling_ratio, feature_type, feature_length, timestamp)

    # dump to file
    topn_dir = '/home/ubuntu/image-search/matches/matrix'
    if not os.path.isdir(topn_dir):
        os.makedirs(topn_dir)
    topn_file = os.path.join(topn_dir, '%d.%s.%d.bin' %
                             (partnerid, feature_type, feature_length))
    print('Dumping matches to file %s' % topn_file)
    pickle.dump(topn_matrix, open(topn_file, 'w'))
...
def dist(a, b, feature_type):
    ...
    # L1 distance
    #print (np.sum (np.abs (a-b)) / len(a))
    # return np.sum (np.abs (a-b)) / len(a)

    # L2 distance
    MULT = 1.0
    return 1.0 - np.dot(a * MULT, b * MULT)


def compute_matches(id1, id2, file1, file2, topn, shrink, feature_type, return_dict):
    ...
    for i in range(n_features_1):
        ...
        dd = np.array([dist(feats_1[i], feats_2[j], feature_type)
    ...
    return_dict[(id1, id2)] = (dx, feature_length)


# build image filename from feature filename
def get_image_name_from_ids(partner_id, internal_id, timestamp):

    return '/opt/old.img/%d/%d__%d.jpg' % (timestamp, partner_id, internal_id)


def build_topn(matches, feature_files, nx, nf, topn, sampling_ratio, feature_type, feature_length, timestamp):
        ...
        for i in range(indx.shape[0]):
            ...
            for j in range(1, indx.shape[1]):
                ...
                if internal_id_1 not in topn_matrix:
                    topn_matrix[internal_id_1] = np.array([internal_id_2])
                else:
                    topn_matrix[internal_id_1] = np.append(
                ...
                img1_name = get_image_name_from_ids(
                    partner_id_1, internal_id_1, timestamp)
                img2_name = get_image_name_from_ids(
                    partner_id_2, internal_id_2, timestamp)
            ...
            if out_img is not None:
                out_filename = '/home/ubuntu/image-search/matches/%d_%d_%s_%04d.jpg' % (
                    partner_id_1, internal_id_1, feature_type, feature_length)
